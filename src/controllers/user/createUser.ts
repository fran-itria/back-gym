...
export default async function createUserController(req: request, res: response) {
    try {
        ...
        if (error.errors && error.errors[0].type == 'unique violation') {
            if (error.errors[0].path == 'user') res.status(404).json({ Error: "Usuario ya registrado" })
            else if (error.errors[0].path == 'email') res.status(404).json({ Error: "Email ya registrado" })
